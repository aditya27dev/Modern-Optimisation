...
runGA <- function(popSize = 20, crossover, noRuns = 30, problem = "feature"){
  ...
  for (i in 1:noRuns){
    ...
    if (problem == "feature")
      ...
      GA <<- ga(type = type, fitness = fitness, distance = route_distance, distMatrix = data, 
               lower = min, upper = max, popSize = popSize, maxiter = maxGenerations,
                     pcrossover=pcrossover, crossover = crossover, pmutation = pmutation, monitor= monitor, seed = i )
